...
This project was entirely built by AI, vibe coded on a whim casually. Claude was asked about organizing a downloads directory and spit out some Python code. I figured "hey, could put that into a project." So here it is.
...
- **Windows Optimized**: Handles UNC paths, long paths (260+ characters), and reserved filenames
...
A comprehensive test suite is included to verify all functionality. The suite includes 104 tests covering unit tests, integration tests, and edge cases.
...
- **Normalize Filename** (14 tests): Version pattern removal, semantic versioning, extension handling, unicode support
- **Similarity Ratio** (11 tests): String comparison logic, case-insensitivity, unicode handling, version-agnostic matching
- **Clustering** (14 tests): File grouping, threshold validation, edge cases, semantic version handling
- **Representative Naming** (8 tests): Folder name generation from file lists, clean name selection
- **Sanitization** (13 tests): Invalid character handling, Windows reserved names, path limits, leading/trailing characters
- **File Exclusion** (6 tests): System file filtering, case-insensitive matching
- **File Organization** (11 tests): Dry-run mode, actual movement, collision handling, error scenarios
- **Input Validation** (6 tests): Threshold and directory validation, user input handling
- **Edge Cases** (7 tests): Unicode files, long names, multiple extensions, file collision handling
- **Helper Assertions** (5 tests): Test utility validation
...
- **Text files**: `file_test_1.txt`, `file_test_2.txt`, `file_test_3.txt`, `readme_v1.txt`, `readme_v2.txt`, `readme_final.txt`
